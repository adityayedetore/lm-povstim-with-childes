...
parser.add_argument('--checkpoint', type=str,  default='model.pt',
                    help='path to the checkpoint to be loaded and evaluated')
...
def evaluate(data_source):
    ...
    with torch.no_grad():
        ...
        for i in range(0, data_source.size(0) - 1, args.bptt):
            # returns the input words and target words (i.e., the
            # input words shifted one to the right
            ...
            # For computing the perplexity, ignore UNK tokens
...
if args.test:
    ...
    # Print number of OOV tokens in the test file being evaluated
